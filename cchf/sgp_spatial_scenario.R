...
sgp_spatial_results <- matrix(NA, total_replication_number, 2)
colnames(sgp_spatial_results) <- c("PCC", "NRMSE")
...
for(repl in 1:total_replication_number){
  ...
  Y_tra <- log(as.matrix(cases[spatial_train_indices, temporal_train_indices]) + 1)
  Y_test <- log(as.matrix(cases[spatial_test_indices, temporal_test_indices]) + 1)
  ...
  sgp_spatial_results[repl,] <- c(cor(matrix(observed, ncol = 1), matrix(predicted, ncol = 1)), sqrt(mean((observed - predicted)^2) / mean((observed - mean(observed))^2)))
}
print(sgp_spatial_results)