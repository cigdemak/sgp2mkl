...
sgp_spatiotemporal_results <- matrix(NA, total_replication_number, 2)
colnames(sgp_spatiotemporal_results) <- c("PCC", "NRMSE")
...
for(repl in 1:total_replication_number){
  ...
  sgp_spatiotemporal_results[repl,] <- c(cor(matrix(observed, ncol = 1), matrix(predicted, ncol = 1)), sqrt(mean((observed - predicted)^2) / mean((observed - mean(observed))^2)))
}
print(sgp_spatiotemporal_results)