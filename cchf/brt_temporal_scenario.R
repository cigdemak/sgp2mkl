...
brt_temporal_results <- c(cor(matrix(observed, ncol = 1), matrix(predicted, ncol = 1)), sqrt(mean((observed - predicted)^2) / mean((observed - mean(observed))^2)))
sprintf("PCC: %f, NRMSE: %f", brt_temporal_results[1], brt_temporal_results[2])