...
p <- sgp2mkl_train(D_temporal_tra_tra, D_spatial_tra_tra, Y_tra, parameters_fix)
...
sgp2mkl_temporal_results <- c(cor(matrix(observed, ncol = 1), matrix(predicted, ncol = 1)), sqrt(mean((observed - predicted)^2) / mean((observed - mean(observed))^2)))
