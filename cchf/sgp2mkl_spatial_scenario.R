require(alabama) || install.packages("alabama") 
...
source(file = "../sgp2mkl/sgp2mkl_train.R")
source(file = "../sgp2mkl/sgp2mkl_test.R")
...
s_ratio_season <- sqrt(0.5)
parameters_fix <- c(s_ratio_latitude, s_ratio_longitude, s_ratio_year, s_ratio_month, s_ratio_season)

replication_number <- 1
sgp2mkl_spatial_results <- matrix(NA, replication_number, 2)
colnames(sgp2mkl_spatial_results) <- c("PCC", "NRMSE")
p <- matrix(NA, replication_number, 10)
...
for(repl in 1:replication_number){
  ...
  temporal_test_indices <- temporal_train_indices
  ...
  D_temporal_tra_tra <- array(c(D_year_tra_tra, D_month_tra_tra, D_season_tra_tra), c(nrow(D_year_tra_tra), ncol(D_year_tra_tra),3))
  ...
  sgp2mkl_spatial_results[repl,] <- c(cor(matrix(observed, ncol = 1), matrix(predicted, ncol = 1)), sqrt(mean((observed - predicted)^2) / mean((observed - mean(observed))^2)))
}
print(sgp2mkl_spatial_results)