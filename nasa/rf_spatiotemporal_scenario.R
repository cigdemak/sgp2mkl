...
for(repl in 1:total_replication_number){
  ...
  state <- randomForest(incidence ~ year + month + latitude + longitude, data = data_tra, ntree = 100000)
  ...
  rf_spatiotemporal_results <-c(cor(matrix(observed, ncol = 1), matrix(predicted, ncol = 1)), sqrt(mean((observed - predicted)^2) / mean((observed - mean(observed))^2)))
}
print(rf_spatiotemporal_results)